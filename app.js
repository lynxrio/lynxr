...
/* ---------- Interactive dot grid ----------
   The Lynx Media Group site's pointer-reactive lattice, ported here and
   retuned for this dark theme. The canvas draws the WHOLE grid — so there
   is exactly one set of dots, perfectly aligned with the CSS lattice it
   replaces — and swells the ones nearest the cursor. Mouse only; touch
   and prefers-reduced-motion keep the static CSS grid.

   It sizes itself through the width/height ATTRIBUTES, never inline
   styles: the strict CSP discards those, and .dot-fx already spans the
   viewport via the stylesheet. */
(function initDotGrid() {
  ...
  // Drawn in device pixels and snapped to the device grid, so the dots
  // stay razor-sharp at any zoom or display density.
  const draw = () => {
    queued = false;
